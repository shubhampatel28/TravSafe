...
const ScoreScreen = () => {
  const { destinationName, stateName, countryName, user, SET_USER_NAME, travelWarning, SET_TRAVEL_WARNING } = React.useContext(
    AppContext
  );
  ...
  const countryCodeUrl = "https://restcountries.eu/rest/v2/name/" + countryName + "?fullText=true"
...
//     } else {
// //Query DB
//       // if user call db else do ntg
//       getCountryCode()
//       if (user) {
//         console.log("User log from score", user)
//       }
//     }
  }, [destinationName]);
  ...
  const getScore = () => {
    const urlforscore = "https://w465h03xn2.execute-api.us-west-2.amazonaws.com/travamplif/score?countryCode=" + countryCode;
    // setTravelScore("")
    axios
      .get(urlforscore)
      .then(function (response) {
  ...
  const getStats = () => {
    const urlforstats = "https://w465h03xn2.execute-api.us-west-2.amazonaws.com/travamplif/covidstats?countryCode=" + countryCode;
    axios
      .get(urlforstats)
      .then(function (response) {
  ...
  const getGraph = () => {
    const urlforstats = "https://w465h03xn2.execute-api.us-west-2.amazonaws.com/travamplif/covidgraph?countryCode=" + countryCode;
    axios
      .get(urlforstats)
      .then(function (response) {
  ...
  if(locationfound == false) {
    return <View><Text>Please select a destination to get started!</Text></View>
  } else {
    return (
      <ScrollView>
        <View style={styles.center}>
          ...
          <Card containerStyle={styles.secondCardContainer}>
            <View style={{ width: 300, alignItems: "left" }}>
              <Text style={styles.secondCardTitleText}>Travel Safety Score for {countryName}</Text>
            </View>
            {/* <Separator /> */}
          ...
          <Card containerStyle={styles.secondCardContainer}>
            <View style={{ width: 300, alignItems: "left" }}>
              ...
              <Text style={styles.secondCardTitleText}>Covid-19 Stats for {countryName}</Text>
            </View>
          ...
          <Card containerStyle={styles.secondCardContainer}>
            <View style={{ width: 300, alignItems: "left" }}>
              <Text style={styles.secondCardTitleText}>Covid Cases Graph for {countryName}</Text>
            </View>
            <View>
